...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome001') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome001'  ORDER BY   c.name,a.name,d.name;
go

case 002 "数据：空间类型更新数据" 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome002') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome002'  ORDER BY   c.name,a.name,d.name;
go

case 003 "数据：空间类型删除数据" 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome003') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome003'  ORDER BY   c.name,a.name,d.name;
go

case 004 "数据：空间类型添加列 " 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome004') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome004'  ORDER BY   c.name,a.name,d.name;
go

case 005 "数据：空间类型删除列 " 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome005') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome005'  ORDER BY   c.name,a.name,d.name;
go

case 006 "数据：空间类型重命名表 " 0
before:
drop table dbo.i2_geome006;
drop table i2_geome006_新表;
go
src_sql:
...
exec sp_rename 'i2_geome006','i2_geome006_新表';
insert into dbo.i2_geome006_新表 values (3,geometry :: STGeomFromText('POLYGON((-122.358 47.653, -122.348 47.649, -122.348 47.658, -122.358 47.658, -122.358 47.653))',4326),geography::STGeomFromText('LINESTRING( -122.360 47.656, -122.343 47.656)', 4326),123.0099);
insert into dbo.i2_geome006_新表(no,c_geography) values(4,geography::STGeomFromText('COMPOUNDCURVE (CIRCULARSTRING (-122.358 47.653, -122.348 47.649, -122.348 47.658), CIRCULARSTRING(-122.348 47.658, -122.358 47.658, -122.358 47.653))',4326));
update dbo.i2_geome006_新表 set c_geography=geography::STGeomFromText('COMPOUNDCURVE (CIRCULARSTRING (-22.358 47.653, -22.348 47.649, -22.348 47.658), CIRCULARSTRING(-22.348 47.658, -22.358 47.658, -22.358 47.653))',4326) where no=2;
delete from i2_geome006_新表 where no=1;
go

tgt_sql: (1)
select * from dbo.i2_geome006_新表 order by no;
select name,xtype,length,isnullable from sys.syscolumns where id = OBJECT_ID('i2_geome006_新表');
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome006_新表') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome006_新表'  ORDER BY   c.name,a.name,d.name;
go

case 007 "数据：空间类型重命名列" 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome007') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome007'  ORDER BY   c.name,a.name,d.name;
go

case 008 "数据：空间列不在最后" 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_geome008') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_geome008'  ORDER BY   c.name,a.name,d.name;
go