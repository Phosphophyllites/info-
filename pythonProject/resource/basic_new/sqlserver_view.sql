...
select b.name as view_schema,a.name as view_name from sys.views a join sys.schemas b on a.schema_id=b.schema_id  where a.name='view_nov';
go

case 002 "通过视图管理表，增删改数据" 0
before:
drop table stuTable;
...
select b.name as view_schema,a.name as view_name from sys.views a join sys.schemas b on a.schema_id=b.schema_id  where a.name='stu_insert_view';
go
...
select b.name as view_schema,a.name as view_name from sys.views a join sys.schemas b on a.schema_id=b.schema_id  where a.name='uv_op_ssxx';
go
