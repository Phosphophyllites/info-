...
case 011 "数据：数字类型增加列带非空约束" 0
before:
drop table dbo.i2_no011;
go
src_sql:
create table dbo.i2_no011 (
    no             int primary key,
    c_int          int default 0 not null,
	c_tinyint	   tinyint check (c_tinyint>=0 and c_tinyint<=255),
	c_smallint	   smallint not null,
...
go
insert into dbo.i2_no011 values (1,-2147483648,0,-32768,-9223372036854775808,-0.000000000000008,-176543.23000000098000008,-300.5670012,-109.1116789,-922337203685477.5808,-214748.3648,1);
insert into dbo.i2_no011 values (2,2147483647,255,-2990,9223372036854775807,0.0000000054,276543.2398,300,208.6789,922337203685477.5807,214748.3647,0);
alter table dbo.i2_no011 add v1 int not null default 0;
alter table dbo.i2_no011 add v2 tinyint not null default 255;
alter table dbo.i2_no011 add v3 smallint not null default -1;
alter table dbo.i2_no011 add v4 bigint not null default 10000;
alter table dbo.i2_no011 add v5 decimal(18,12) not null default 0.0000008;
alter table dbo.i2_no011 add v6 numeric(12,8) not null default 999.00001234;
alter table dbo.i2_no011 add v7 float not null default -1.004567;
alter table dbo.i2_no011 add v8 real not null default 1.12345;
alter table dbo.i2_no011 add v9 money not null default -111.678;
alter table dbo.i2_no011 add v10 smallmoney not null default 123.678;
alter table dbo.i2_no011 add v11 bit not null default 1;
insert into dbo.i2_no011 values ( 3,300,55,10003,50003,-167.45,876543.2398,300.45,500.6789,100.23,300.45,0,-214748,10,1,-9112345,-1234.005678,7890.123056,-1261.3546,10089.001005,1789.123,87654.123,1);
insert into dbo.i2_no011(no,c_int,c_smallint,c_bigint) values (4,-400,100,50005);
go

tgt_sql:
select * from dbo.i2_no011 order by no;
select name,xtype,length,isnullable from sys.syscolumns where id = OBJECT_ID('i2_no011');
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_no011') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_no011'  ORDER BY   c.name,a.name,d.name;
go