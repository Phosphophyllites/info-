case 001 "数据：sql_variant类型插入数据" 0
before:
...
go
insert into dbo.i2_variant001 values (1,newid(),'fang56789','AA101',N'GH𧿹',N'䒦上海英方股份有限公司测试用例1',-2147483648,-32768,81,-9223372036854775808,-0.0000000008,-176543.2398,-300.12,-109.6789,-110.23,-101.45,1,
...
insert into dbo.i2_variant001 values (2,newid(),'yuan56789','AA102',N'䗪jjj',N'上海英方股份䗪有限公司测试用例2',10,9999,91,67891,2020.88,500.6789,300.45,500.6789,100.23,300.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
insert into dbo.i2_variant001 values (3,newid(),'chang56789','AA103',N'GHjjjj䗪jjjjj',N'上海英方股份有限公司测试用例4䗪',10,9999,101,67891,2020.88,500.6789,300.45,500.6789,100.23,300.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant001') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant001'  ORDER BY   c.name,a.name,d.name;
go

case 002 "数据：sql_variant类型更新数据" 0
before:
...
go
insert into dbo.i2_variant002 values (1,newid(),'fang56789','AA101',N'GH𧿹',N'䒦上海英方股份有限公司测试用例1',-2147483648,-32768,81,-9223372036854775808,-0.0000000008,-176543.2398,-300.12,-109.6789,-110.23,-101.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
CONVERT (datetimeoffset,SYSDATETIMEOFFSET()),
0x1f,0x111111111,'/1/2/3/');
insert into dbo.i2_variant002 values (2,newid(),'yuan56789','AA102',N'䗪jjj',N'上海英方股份䗪有限公司测试用例2',10,9999,91,67891,2020.88,500.6789,300.45,500.6789,100.23,300.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
CONVERT (datetimeoffset,SYSDATETIMEOFFSET()),
0x2f,0x111111111,'/1/2/3/');
insert into dbo.i2_variant002 values (3,newid(),'chang56789','AA103',N'GHjjjj䗪jjjjj',N'上海英方股份有限公司测试用例4䗪',10,9999,101,67891,2020.88,500.6789,300.45,500.6789,100.23,300.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
CONVERT (datetimeoffset,SYSDATETIMEOFFSET()),
0x3f,0x111111111,'/1/2/3/');
insert into dbo.i2_variant002(no,c_varchar,c_nvarchar,c_tinyint) values ( 4,'AA104',N'Hjjjj䗪jjjjj',111);
update dbo.i2_variant002 set c_nvarchar=N'测试用例4䗪',c_float=201.34,c_datetime='2019-01-28 10:10:10.333',c_hierarchyid='/1/4/6/',c_binary=0x7f,c_bigint=56789 where no=2;
go
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant002') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant002'  ORDER BY   c.name,a.name,d.name;
go

case 003 "数据：sql_variant类型删除数据" 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant003') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant003'  ORDER BY   c.name,a.name,d.name;
go

case 004 "数据：sql_variant类型添加列 " 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant004') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant004'  ORDER BY   c.name,a.name,d.name;
go

case 005 "数据：sql_variant类型删除列 " 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant005') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant005'  ORDER BY   c.name,a.name,d.name;
go

case 006 "数据：sql_variant类型重命名表 " 0
before:
drop table dbo.i2_variant006;
drop table dbo.i2_variant006_新表;
go
src_sql:
...
CONVERT (datetimeoffset,SYSDATETIMEOFFSET()),
0x2f,0x111111111,'/1/2/3/');
exec sp_rename 'dbo.i2_variant006','i2_variant006_新表';
insert into i2_variant006_新表 values (3,newid(),'chang56789','AA103','GHjjjjjjjjj','上海英方股份有限公司测试用例4',10,9999,101,67891,2020.88,500.6789,300.45,500.6789,100.23,300.45,1,
convert(date,SYSDATETIME()),
convert(time,SYSDATETIME()),
convert(datetime,'1999-12-12 14:31:56.333',20),
convert(datetime2,'2048-06-01 20:51:24.99999',21),
...
CONVERT (datetimeoffset,SYSDATETIMEOFFSET()),
0x3f,0x111111111,'/1/2/3/');
insert into i2_variant006_新表(no,c_varchar,c_tinyint) values ( 4,'AA104',111);
update dbo.i2_variant006_新表 set c_date='2020-01-02',c_datetime='2021-02-03 17:18:19' where no=2;
delete from i2_variant006_新表 where no=1;
go

tgt_sql: (1)
select no,convert(uniqueidentifier,c_unique),convert(char(20),c_char20),convert(varchar(50),c_varchar),convert(nchar(50),c_nchar50),convert(nvarchar(50),c_nvarchar),convert(int,c_int),convert(smallint,c_smallint),convert(tinyint,c_tinyint),convert(bigint,c_bigint),convert(decimal(10,4),c_decimal),convert(numeric(10,4),c_numeric),convert(float,c_float),convert(real,c_real),convert(money,c_money),convert(smallmoney,c_smallmoney),convert(bit,c_bit),convert(date,c_date),convert(time,c_time),convert(datetime,c_datetime),convert(datetime2,c_datetime2),convert(smalldatetime,c_smalldatetime),convert(datetimeoffset,c_datetimeoffset),convert(binary,c_binary),convert(varbinary,c_varbinary),convert(varchar(50),c_hierarchyid) from dbo.i2_variant006_新表 order by no;
select name,xtype,length,isnullable from sys.syscolumns where id = OBJECT_ID('i2_variant006_新表');
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant006_新表') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant006_新表'  ORDER BY   c.name,a.name,d.name;
go

case 007 "数据：sql_variant类型重命名列 " 0
before:
...
select name,xtype from sys.sysobjects where parent_obj= OBJECT_ID('i2_variant007') order by name;
select 索引名称=a.name ,表名=c.name ,索引字段名=d.name ,索引字段位置=d.colid  FROM  sysindexes   a JOIN   sysindexkeys   b   ON   a.id=b.id   AND   a.indid=b.indid JOIN   sysobjects   c   ON   b.id=c.id  JOIN   syscolumns   d   ON   b.id=d.id   AND   b.colid=d.colid WHERE   a.indid   NOT IN(0,255) AND   c.name='i2_variant007'  ORDER BY   c.name,a.name,d.name;
go